...
public class Q26_RemoveDuplicatesFromSortedArray {
    ...
    private static void testCase() {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            Arrays.sort(nums);
            // 正确结果
            int[] correct = Arrays.stream(nums).distinct().toArray();

            // 我的结果
            int length = solution.removeDuplicates(nums);
            int[] res = Arrays.copyOf(nums, length);

            if (ArrayUtils.notEquals(res, correct)) {
                System.out.println(Arrays.toString(nums));
                System.out.println("正确答案：" + Arrays.toString(correct));
                System.out.println("我的答案：" + Arrays.toString(res));
